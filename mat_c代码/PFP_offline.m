function [SpikeC,W,residue,huifu_ECG,huifu,peelcount,numc]=PFP_offline(x,Spike,param,ann)
if nargin==0
...
if (~isfield(param,'peakinterval'))
    param.peakinterval=150/1000*param.fs;
end
...
if param.auto==1
    ...
    while count<=param.peelnum
        ran=randi(param.df)
        [z,~,Cond]=yanchibaihua3(residue, ran,0); 
        if Cond>1e10
            disp(['warn:the Cond is too large!',num2str(Cond)]);
            return
        end
        figure
        while size(B,2)<2 ||1-abs(B(:,end)'*B(:,end-1))<1e5
            if param.ini==1
...
%                 yc(((i-1)*param.outnum+1):i*param.outnum,:)=fastic_parallel3(z(1:floor((0.5+0.5*(rand>0.5))*size(z,1)),:),param.outnum);
                ...
                extend=rand;
%                 [yc,B1]=fastica_single(z,B);          
                [yc,B1]=fastica_single(z(1:floor((0.5+0.25*(extend>=0.5)+0.25*(extend>=0.75))*size(z,1)),:),B);
%                 [yc,B1]=fastica_single(z,B);
                
                if size(B,2)>=2
                    thresholdF=1-abs(B(:,end)'*B(:,end-1))
                end
                tt=ones(1,length(ann));
...
%                 while(1)
%                 de=input('Please input "0" to delete it):','s');
...
%                 
%                 % 提取Spike
% %                 [F1,F2,yc]=getspike(yc,1,param.fs);
%                 
%                 % cov系数筛选
%                 [B1,PT1]=revise(yc,z,B1,param.fs);
%                 %是否需要再通过阈值筛选
%                 if ~isempty(B1)
%                     B1(end+1:size(z,1))=0;
                    B=[B B1];
...
% FastICA之后的筛选
function [w1,PT]=revise(yc,xw,w,fs)%pwei1zehuatu
% 初始变异系数
cov0=1e6;
cov1=1e5;
w1=w;
...
while cov1<cov0
    % b.Estimate the pulse train,选取幅值重心最大的一类？
    sc=w1'*xw;
    % c.Set CoVn?1= CoVn,calculate CoVn of PTn,
    %但是只能找到最大的那一类，能否通过与聚类的点进行匹配，选择数量最多的那一类，弱化幅值的影响
%     [F1,F2,sc]=getspike(sc,1,param.fs);
%     [PT,S,sc,w]=clusterSpike(sc,w,param.fs);
[PT,S,sc,w1]=peakCluster(sc,w1,fs);
%     信号长度：5s，wi：1*1000
    if isempty(PT)
        w1=[];
        PT=[];
    ...
    function [coef_s,coef_w]=cal_cov(yc,PT,fs)
        a=diff(PT);
...
function [Fmax,S,x,wx]=clusterSpike(x,wx,fs)%解决符号不一致的一些问题
win=fix(fs/100);
x_jd=x(2*win+1:end-2*win);

% 归一化
x_jd=x_jd/abs(max(x_jd));
...
tm = 1:mm1;
figure
findpeaks(yp,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',interval,'Annotate','extents');
[pks1,L1,w1,proms1] =findpeaks(yp,'MinPeakProminence',th,'MINPEAKDISTANCE',interval,'Annotate','extents');
[pks2,L2,w2,proms2] =findpeaks(yp,'MinPeakProminence',outlier,'Annotate','extents');
[L3,loc3]=setdiff(L1,L2);%返回在L1中，不在L2中的元素
pks3=pks1(loc3);
w3=w1(loc3);
proms3=proms1(loc3);
figure
findpeaks(yn,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',interval,'Annotate','extents')
[pks1i,L1i,w1i,proms1i] =findpeaks(yn,'MinPeakProminence',th,'MINPEAKDISTANCE',interval,'Annotate','extents');
[pks2i,L2i,w2i,proms2i] =findpeaks(yn,'MinPeakProminence',outlier,'Annotate','extents');
[L3i,loc3i]=setdiff(L1i,L2i);%返回在L1中，不在L2中的元素
pks3i=pks1i(loc3i);
w3i=w1i(loc3i);
proms3i=proms1i(loc3i);
...
%相对指标
Sc(:,1)=w;%宽
Sc(:,2)=abs(max(Spike,[],2)-min(Spike,[],2));%高
% 20ms内的积分
Sc(:,3)=sqrt(mean(diff(Spike,1,2).^2,2));


%标准化
% Sc_map(:,1)=mapstd(Sc(:,1)',0,1);
% Sc_map(:,2)=mapstd(Sc(:,2)',0,1);
% Sc_map(:,3)=mapstd(Sc(:,3)',0,1);
Sc_map=Sc;

[C]=valley_seeking(Sc_map,30,0.6,4);
figure;
for ii=1:max(C)
    scatter(Sc_map(C==ii,1),Sc_map(C==ii,2)); hold on
end
scatter(Sc_map(C==-1,1),Sc_map(C==-1,2));hold on
%             pause(3);close
...
% function [F,F2,x,Spike,Sc,L,C]=getspike(x,p,fs)%解决符号不一致的一些问题
% win=fix(fs/100);
% x_jd=x(2*win+1:end-2*win);
% yp=x_jd;
% % yp(yp<0)=0;
...
% outlier=20;
% % hold on
% % line([0 mm],[2 2],'linestyle','--', 'Color','r', 'LineWidth', th);
% 
% mm1=length(yp);
% tm = 1:mm1;
% figure
% findpeaks(yp,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',param.peakinterval,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents');
% [pks1,L1,w1,proms1] =findpeaks(yp,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',param.peakinterval,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents');
% [pks2,L2,w2,proms2] =findpeaks(yp,tm,'MinPeakProminence',outlier,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents');
% [L3,loc3]=setdiff(L1,L2);%返回在L1中，不在L2中的元素
% pks3=pks1(loc3);
% w3=w1(loc3);
% proms3=proms1(loc3);
% figure
% findpeaks(yn,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',param.peakinterval,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents')
% [pks1i,L1i,w1i,proms1i] =findpeaks(yn,tm,'MinPeakProminence',th,'MINPEAKDISTANCE',param.peakinterval,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents');
% [pks2i,L2i,w2i,proms2i] =findpeaks(yn,tm,'MinPeakProminence',outlier,'MaxPeakWidth',50/1000*param.fs,'Annotate','extents');
% [L3i,loc3i]=setdiff(L1i,L2i);%返回在L1中，不在L2中的元素
% pks3i=pks1i(loc3i);
% w3i=w1i(loc3i);
% proms3i=proms1i(loc3i);
...
% 
% % figure;
% %             scatter(Sc_map(:,1),Sc_map(:,2));
% %             xlabel('标准化宽度')
% %             ylabel('高度')
% [C]=valley_seeking(Sc_map,20,0.5,3);
%             figure;
%             for ii=1:max(C)
%                 scatter(Sc_map(C==ii,1),Sc_map(C==ii,2)); hold on
%             end
...
%     end
    ...
    function [y_out,w2]=fastica_single(z,B,wini)
        %F为解混向量
        [m,T]=size(z);
        if nargin<3  %nargin为"number of input arguments"的缩写。 在matlab中定义一个函数时， 在函数体内部， nargin是用来判断输入变量个数的函数。
            %w2为最大平方和的白化向量
            [index,loc]=max(sum(z.*z));
            w2=z(:,loc);
            w2=w2/norm(w2);%randn：产生均值为0，方差σ^2 = 1，标准差σ = 1的正态分布的随机数或矩阵的函数。sqrtm求矩阵的平方根
        else
            w2=z(:,wini);
            w2=w2/norm(w2); %w初始权矢量（任意）
        end
        %随机生成w1
        w1=randn(m,1);
        w1=w1/norm(w1);
        
%         w2=mean(z(:,F),2);w2=w2/norm(w2);
        c=1;
        
        while (abs(1-abs(w1'*w2))>1e-6)&&(c<2000)
            w1=w2;
            w2=(1/T*g(w1'*z)*z')'-mean(gd(w1'*z))*w1;
%             w2=w2/norm(w2);
            % gram-schmit 正交化
            if ~isempty(B)
                w2=w2- B(1:size(w2,1),:) * B(1:size(w2,1),:)' * w2;
            end
            w2 = w2 / norm(w2);
            c=c+1;            
        end
