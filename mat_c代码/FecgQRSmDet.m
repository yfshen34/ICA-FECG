function [qrsRef,indexm]=FecgQRSmDet(Se,fs,cName,graph,dbFlag,saveFig,qrsAf)
% ---------------------------------------------------------------------------------------------
...
%         subplot(ns,1,is), plot(vtime,X(:,is),'red');
%         subplot(ns,1,is), plot(vtime,X(:,is),'blue');
        wgmi1= min(X(:,is)) -2;
        wgma1= max(X(:,is)) +2;
        ylim([wgmi1, wgma1]);
        set(gca,'YTick',[-5 0 5])
...
% mQRS在2 s宽的窗口中至少会发生一次，在0.2 s宽的窗口中只会发生几次，而在这两种宽度的窗口中都可能出现噪声。
% 另一方面，零星的mecg应该出现在一些8秒宽的窗户，而只有少数2秒宽的窗户
% 变异系数+RR间隔
for is=1:ns, 
...
%     if(learning), plot(qrsAf, [RRafs(1);RRafs],'r.-'); end% RRafs为参考信号fecg的RR间距
%     plot(qrsRef/fs, [RRs(1);RRs],'b.-');% qrsRef为计算得到的mecg的RR间距
%     ylim([0.3, 2]);% y轴范围为0.3-2
