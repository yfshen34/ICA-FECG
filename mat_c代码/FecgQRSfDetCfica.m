function qrsFes=FecgQRSfDetCfica(X,fs,cName,qrsM,graph,dbFlag,saveFig,saveFigRRf,qrsAfs)
%---------------------------------------------------------------------------------------------
...
% [b,a]= butter(1,Wn);
% decgBp=filter(b,a,decgr);

% -----   absolute derivative ------
adecg=abs(decgr);
% S=searchspike_in(adecg',40,0.3*fs);
...
for ic=1:ns
    ...
    xic=X(:,ic);%标准化
    ecgic=ecg(:,ic);%标准化
    decgic=decgr(:,ic);%差分滤波器
%     decgBpic=decgBp(:,ic);% 差分滤波器+1-6Hz滤波
    adecgic=adecg(:,ic);% 差分滤波器+绝对值
    adecgBpic=adecgBp(:,ic);% 差分滤波器+绝对值+1-6Hz滤波
...
%     end
    ...
    adRR=abs(diff(RRfs1));% adRR：预选择RR间隔之间的差
...
%     if(graph), figure, plot(qrsFp1(3:end)/fs,mvadRR); end% 预检测的RR间隔5个之和
    thad=0.4;
    RRoklen=0;
    while(~RRoklen)
        for j=1:4
            isRRok=[]; ieRRok=[];
            jint=1;  RRokFlag=0;
            for i=1:length(mvadRR)%如果间隔处于众数+-0.1
...
% 
% if(learning && graph)
%     PlotSgnMrkNc(ecgs, {qrsAfs*fs,qrsFes*fs}, fs, [cName,': fecg, ann.(m) & det.(b) qrs']); 
% end
% if(saveFig)
