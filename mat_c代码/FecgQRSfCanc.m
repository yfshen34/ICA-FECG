function [Xr,Xf]=FecgQRSfCanc(X,qrsM,fs,cName,graph,dbFlag,saveFig,qrsAfs)
% ---------------------------------------------------------------------------------------------
...
npd=fix(min(0.35, 0.8*(RRmean-0.1))*fs);  % number of samples after the qrs reference
npt=1+npp+npd;
...
if(qrsM(1)-npqp < 1), qi=2; 
    ...
    npxp=round(max(0,npp+1-qrsM(1)));  % number of samples added to left of each signal
...
if(size(RRs,1)>4)
    ...
    if(qrsM(qf)+0.85*fs*median(RRs) < size(X,1))
        npep=fix(max(0.1*fs, 0.15*fs*mean(RRs)));
        Xx(end-npep+1:end,:)=repmat(Xx(end-npep-1,:),npep,1);
    end
end
npxd=round(max(0,qrsM(qf)+npd-ndt));  % number of samples added to right of each signal
...
fw=round(fw);
iw=round(iw);
A=zeros(npt,nqe);%心电序列周期
wwg=weightFun2(npp,npd,fs);% 不规则梯形窗口
Xc=zeros(ndtx,ns);
for is=1:ns
    for iq=1:nqe
        iwq=iw(iq); fwq=fw(iq);
    ...
    % The matrix A has dimension (npt, nqe) with npt> nqe
...
%    fprintf('%f,  ', diag(S));   fprintf('\n');
    ...
    % else
    %     nds=4;
    % end     % select the number of singular values  <===
...
%    tic
    Ar=U(:,1:nds)*S(1:nds,1:nds)*V(:,1:nds)';
