...
% 重新跑一遍block的处理
    for i = 0 : numBlock-1
        dataRange = 1 + floor(i*step) : min(n, floor((i+1)*step));
        xx = data(:,dataRange);
        state = dynamicOrica(xx, state, dataRange, adaptiveFF, evalConvergence, nlfunc);
 ...
 % 画出来看效果
  ...
  % 等待人工确认，按空格继续
%     disp(['当前a = ', num2str(a), '，按空格继续，按Ctrl+C停止']);
%     pause; % 等待你按任意键继续
% aList = 3.5:0.1:5.0; % 超参数a从0.5扫到2
% for ai = 1:length(aList)
%     a = aList(ai);
%     mynlfunc = @(y) 1-2./(1+a*exp(-30*y)); % 把超参数a放进去
% 
%     % 重新初始化state，防止累积
%     state.lambda_k      = zeros(1,40);
%     state.minNonStatIdx = [];
%     state.counter       = 0;
%     state.Rn = [];
%     state.nonStatIdx = [];
